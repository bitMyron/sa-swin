...
def rot_rand(args, x_s, fixed=False):
    img_n = x_s.size()[0]
...
def rot_rand_for_double(args, x_s, x_flip_s, fixed=False):
    img_n = x_s.size()[0]
    x_aug = x_s.detach().clone()
    x_flip_aug = x_flip_s.detach().clone()
    device = torch.device(f"cuda:{args.local_rank}")
    x_rot = torch.zeros(img_n).long().to(device)
    for i in range(img_n):
        x = x_s[i]
        ...
        if orientation == 0:
            ...
            x = x.rot90(1, (2, 3))
            x_flip = x_flip.rot90(1, (2, 3))
        elif orientation == 2:
            x = x.rot90(2, (2, 3))
            x_flip = x_flip.rot90(2, (2, 3))
        elif orientation == 3:
            x = x.rot90(3, (2, 3))
            x_flip = x_flip.rot90(3, (2, 3))
        x_aug[i] = x
        x_flip_aug[i] = x_flip
        x_rot[i] = orientation
    return x_aug, x_flip_aug, x_rot
