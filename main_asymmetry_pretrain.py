...
from losses.loss import AsymmetrySimilarityLoss
...
parser.add_argument("--warmup_epochs", default=50, type=int, help="warmup steps")
...
parser.add_argument("--loss_type", default="siamese", type=str)
...
parser.add_argument("--class_weights", default="[1,2]", type=str, help="number of classification target classes")
...
def train_epoch(model, loader, optimizer, scaler, epoch, loss_func, args):
    ...
    for idx, batch_data in enumerate(loader):
        ...
        with autocast(enabled=args.amp):
            ...
            loss = loss_func(y_contrastive, y_flip_contrastive, target)
        if args.amp:
        ...
        if args.rank == 0:
            print(
                ...
                'current_lr: {:.6f}:'.format(optimizer.param_groups[0]['lr']),
    ...
    with torch.no_grad():
        for idx, batch_data in enumerate(loader):
            ...
            with autocast(enabled=args.amp):
                y_contrastive = model(data)
                y_flip_contrastive = model(data_flip)
                loss = loss_function(y_contrastive, y_flip_contrastive, target)
            if args.distributed:
...
def main():
    ...
    def train(args, start_epoch, train_loader, val_best, scaler, scheduler):
        ...
        for epoch in range(start_epoch, args.max_epochs):
            # print('learning rate for epoch %d: ' % epoch, scheduler.get_last_lr())
            # print('setting lr: ', args.lr)
            if args.distributed:
            ...
            if (epoch + 1) % args.val_every == 0:
                ...
                if args.rank == 0:
                    ...
                    save_ckp(checkpoint, logdir + "/model_latest_checkpoint.pt")
    ...
    if args.lrdecay:
        if args.lr_schedule == "warmup_cosine":
            scheduler = LinearWarmupCosineAnnealingLR(optimizer, warmup_start_lr=args.lr/args.warmup_epochs, warmup_epochs=args.warmup_epochs, max_epochs=args.max_epochs)
    ...
    loss_function = AsymmetrySimilarityLoss(method=args.loss_type)
    if args.distributed:
