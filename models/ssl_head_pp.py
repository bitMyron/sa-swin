...
from monai.networks.nets.swin_unetr_pp import SwinTransformer as SwinViT
...
class SSLHead(nn.Module):
    def __init__(self, args, upsample="vae", dim=768):
        ...
        self.rotation_pre = nn.Identity().to('cuda:3')
        self.rotation_head = nn.Linear(dim, 4).to('cuda:3')
        self.contrastive_pre = nn.Identity().to('cuda:3')
        self.contrastive_head = nn.Linear(dim, 512).to('cuda:3')
        if upsample == "large_kernel_deconv":
            self.conv = nn.ConvTranspose3d(dim, args.in_channels, kernel_size=(32, 32, 32), stride=(32, 32, 32)).to('cuda:3')
