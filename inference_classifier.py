        ...
        with torch.no_grad():
            for idx, batch_data in enumerate(val_dataloader):
                ...
                with autocast(enabled=args.amp):
                    ...
                    auc_metric(y_pred=probs, y=target)
                for id, prob, label in zip(ids.tolist(), probs.tolist(), labels.tolist()):
                    eval_results.append({'id': id, 'label': label, 'pred':prob[-1]})

        auc_val = auc_metric.aggregate()
        print("Validation aggregated, AUC is: ", auc_val)
        pathlib.Path(args.output_dir).parent.mkdir(parents=True, exist_ok=True)
