...
from losses.loss import Loss_PP
from models.ssl_head_pp import SSLHead
from optimizers.lr_scheduler_pretrain import WarmupCosineSchedule
from torch.cuda.amp import GradScaler, autocast
from torch.nn.parallel import DistributedDataParallel
from torch.utils.tensorboard import SummaryWriter
from utils.data_utils_pretrain import get_loader
...
# def join_layers(pretrain_model):
#     layers = [
#         *vision_model.swinViT,
#         lambda x: torch.flatten(start_dim=2, end_dim=4),
#         *vision_model.rotation_pre,
#         *vision_model.rotation_head,
#         *vision_model.contrastive_pre,
#         *vision_model.contrastive_head,
#         *vision_model.conv,
#     ]
#     return layers
...
def main():
    ...
    def train(args, global_step, train_loader, val_best, scaler):
        ...
        for step, batch in enumerate(train_loader):
            ...
            x1_augment = x1_augment.to('cuda:0')
            x2_augment = x2_augment.to('cuda:0')
            with autocast(enabled=args.amp):
                rot1_p, contrastive1_p, rec_x1 = model(x1_augment)
                rot2_p, contrastive2_p, rec_x2 = model(x2_augment)
                rot_p = torch.cat([rot1_p, rot2_p], dim=0).to('cuda:3')
                rots = torch.cat([rot1, rot2], dim=0).to('cuda:3')
                imgs_recon = torch.cat([rec_x1, rec_x2], dim=0).to('cuda:3')
                imgs = torch.cat([x1, x2], dim=0).to('cuda:3')
            ...
            if args.distributed:
                ...
                print("Step:{}/{}, Loss:{:.4f}, Time:{:.4f}".format(global_step, args.num_steps, loss.item(),
                                                                    time() - t1))
    ...
    def validation(args, test_loader):
        ...
        with torch.no_grad():
            ...
            for step, batch in enumerate(test_loader):
                val_inputs = batch["image"].to('cuda:0')
                x1, rot1 = rot_rand(args, val_inputs)
                x2, rot2 = rot_rand(args, val_inputs)
                x1_augment = aug_rand(args, x1)
                x2_augment = aug_rand(args, x2)
                with autocast(enabled=args.amp):
                    rot1_p, contrastive1_p, rec_x1 = model(x1_augment.to('cuda:0'))
                    rot2_p, contrastive2_p, rec_x2 = model(x2_augment.to('cuda:0'))
                ...
                print("Validation step:{}, Loss:{:.4f}, Loss Reconstruction:{:.4f}".format(step, loss.item(),
                                                                                           loss_recon.item()))
    ...
    loss_function = Loss_PP(args.batch_size * args.sw_batch_size, args)
