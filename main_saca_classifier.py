...
from utils.data_utils_classifier import get_loader_with_symmetric_whole
from utils.utils import AverageMeter, distributed_all_gather
import resource
import wandb
import json
from models.ssl_head import SACAClassifierHead
...
parser.add_argument("--patch_size", default=2, type=int, help="patch size for embedding")
...
def val_epoch(model, loader, epoch, loss_function, args, model_inferer=None):
    ...
    with torch.no_grad():
        for idx, batch_data in enumerate(loader):
            ...
            data, target = data.cuda(args.rank), target.cuda(args.rank)
            data_flip = batch_data["image_symmetrical"].cuda(args.rank)
            with autocast(enabled=args.amp):
                logits = model(data, data_flip)
...
def main():
    ...
    train_loader, test_loader = get_loader_with_symmetric_whole(args)
