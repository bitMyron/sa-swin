...
class RandSpatialCropWithSymmetricCounterpart(Randomizable, Crop):
    """
    Crop image with random size or specific size ROI. It can crop at a random position as center
    or at the image center. And allows to set the minimum and maximum size to limit the randomly generated ROI.
    ...
    def __init__(
        ...
        self._size: Sequence[int] | None = None
        self._slices: tuple[slice, ...]
        self._slices_symmetric: tuple[slice, ...]
    ...
    def randomize(self, img_size: Sequence[int]) -> None:
        ...
        if self.random_center:
            ...
            # self._slices = (slice(0, 96), slice(0, 96), slice(0, 96))
            new_start = img_size[0] - self._slices[0].stop
            new_stop = img_size[0] - self._slices[0].start
            new_slice = slice(new_start, new_stop)
            self._slices_symmetric = (new_slice, self._slices[1], self._slices[2])

    def __call__(self, img: torch.Tensor, randomize: bool = True) -> Tuple[Tensor, Tensor] | Tensor:  # type: ignore
        """
        Apply the transform to `img`, assuming `img` is channel-first and
        slicing doesn't apply to the channel dim.
        ...
        if self._size is None:
            raise RuntimeError("self._size not specified.")
        if self.random_center:
            return super().__call__(img=img, slices=self._slices), super().__call__(img=img,
                                                                                    slices=self._slices_symmetric)
        cropper = CenterSpatialCrop(self._size)
        return super().__call__(img=img, slices=cropper.compute_slices(img.shape[1:])), super().__call__(img=img, slices=cropper.compute_slices(img.shape[1:]))
...
class RandSpatialCropSamplesWithSymmetricCounterpart(Randomizable, TraceableTransform, MultiSampleTrait):
    """
    Crop image with random size or specific size ROI to generate a list of N samples.
    It can crop at a random position as center or at the image center. And allows to set
    the minimum size to limit the randomly generated ROI.
    ...
    backend = RandSpatialCropWithSymmetricCounterpart.backend
    ...
    def __init__(
        ...
        if num_samples < 1:
            raise ValueError(f"num_samples must be positive, got {num_samples}.")
        self.num_samples = num_samples
        self.cropper = RandSpatialCropWithSymmetricCounterpart(roi_size, max_roi_size, random_center, random_size)
    ...
    def __call__(self, img: torch.Tensor) -> Tuple[list[torch.Tensor], list[torch.Tensor]] :
        """
        Apply the transform to `img`, assuming `img` is channel-first and
        cropping doesn't change the channel dim.
        """
        ...
        orig_size = img.shape[1:]
        for i in range(self.num_samples):
            cropped, cropped_symmetry = self.cropper(img)
            if get_track_meta():
                cropped.meta[Key.PATCH_INDEX] = i  # type: ignore
                cropped_symmetry.meta[Key.PATCH_INDEX] = i  # type: ignore
                self.push_transform(cropped, orig_size=orig_size, extra_info=self.pop_transform(cropped, check=False))
                self.push_transform(cropped_symmetry, orig_size=orig_size, extra_info=self.pop_transform(cropped_symmetry, check=False))
            ret.append(cropped)
            # Flip the patch to match the symmetric patch orientation
            cropped_symmetry = torch.flip(cropped_symmetry, dims=(1,))
            ret_symmetry.append(cropped_symmetry)
        return ret, ret_symmetry
