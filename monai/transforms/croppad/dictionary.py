...
from monai.transforms.croppad.array import (
    ...
    SpatialCrop,
    SpatialPad,
    RandSpatialCropSamplesWithSymmetricCounterpart
)
...
class RandSpatialCropSamplesdWithSymmetricCounterpart(Randomizable, MapTransform, MultiSampleTrait):
    """
    Dictionary-based version :py:class:`monai.transforms.RandSpatialCropSamples`.
    Crop image with random size or specific size ROI to generate a list of N samples.
    It can crop at a random position as center or at the image center. And allows to set
    ...
    backend = RandSpatialCropSamplesWithSymmetricCounterpart.backend
    ...
    def __init__(
        ...
        MapTransform.__init__(self, keys, allow_missing_keys)
        self.cropper = RandSpatialCropSamplesWithSymmetricCounterpart(roi_size, num_samples, max_roi_size, random_center, random_size)
    ...
    def __call__(self, data: Mapping[Hashable, torch.Tensor]) -> list[dict[Hashable, torch.Tensor]]:
        ...
        for key in self.key_iterator(dict(data)):
            self.cropper.set_random_state(seed=self.sub_seed)
            crop_org, crop_symmetry = self.cropper(data[key])
            for i, (im, im_sym) in enumerate(zip(crop_org, crop_symmetry)):
                ret[i][key] = im
                ret[i][f"{key}_symmetrical"] = im_sym
        return ret
